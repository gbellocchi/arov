...
if [ -f "$test_path/slm_conv" ]; then
    ...
    error_exit ">> ERROR: No SLM file converter has been found in the test directory. Aborting."
fi
...
if [ -d "$dir_slm_files" ]; then
    ...
    error_exit ">> ERROR: No SLM file directory has been found. Aborting."
fi
...
if [ -f "$app_path/${app_name}_l1.slm" ]; then
    ...
    -w 32 -P 32 -S 1 -n 1024 -s 0x10000000 -F l1_%01S_%01P.slm
else
    error_exit ">> ERROR: Missing L1 binaries at $app_path/. Aborting."
fi
...
if [ -f "$app_path/${app_name}_l2.slm" ]; then
    ...
    error_exit ">> ERROR: Missing L2 binaries at $app_path/. Aborting."
fi
